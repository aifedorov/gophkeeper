...
func TestRepository_GetCredential(t *testing.T) {
	t.Parallel()

	t.Run("gets credential successfully", func(t *testing.T) {
		t.Parallel()
		...
		mockQuerier := NewMockQuerier(ctrl)
		mockQuerier.EXPECT().
			GetCredential(ctx, GetCredentialParams{
				ID:     credID,
				UserID: userID,
			}).
			Return(expectedDBCred, nil)
